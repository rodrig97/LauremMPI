...
class escalafon extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->library(array(
            ...
            'App/personaslegajo',
            'App/estudioslegajo',
            'App/capacitacionlegajo',
            'App/experiencialegajo',
            'App/meritoslegajo',
            'App/demeritoslegajo',
            'App/familiarlegajo',
        ));
    ...
    public function ui_full_info()
    {
        ...
        $this->load->view(
            'escalafon/v_registrar_fullinfo',
            array(
                'anios'         => $anios,
                'pers_info'      => $pers_info,
                'afps'           => $afps,
    ...
    public function ui_legajo()
    {
        $personas =  $this->persona->get_trabajadores_legajo();
        ...
        $this->load->view('escalafon/p_legajo', array(
            'tipo_empleados' => $tipos_empleado,
            'dependencias' => $dependencias,
            'cargos' => $cargos,
            'personas' => $personas,
    ...
    public function ui_legajo_full()
    {
        ...
        if (count($datos_personales) < 1) {
            $datos_personales = $this->personaslegajo->insert_pers_id($indiv_id);
            if ($datos_personales) {
                $datos_personales = $this->personaslegajo->get_pers_id($indiv_id);
            } else {
    ...
    public function ui_legajo_estudios()
    {
        ...
        $iperstipoestudid        = trim($this->input->post('iperstipoestudid'));

        if ($iperstipoestudid > 0) {
            switch ($accion) {
                case 'actualizar':
                    $estudios = $this->estudioslegajo->listxiperstipoestudid($iperstipoestudid);
                    $this->load->view(
                        'escalafon/v_legajo_modal_estudios',
                        array(
                            'accion' => $accion,
                            'indiv_id' => $indiv_id,
                            'ipersid' => $ipersid,
    ...
    public function estudios_accion()
    {
        ...
        switch ($accion) {
            case 'agregar':
                $estudios = $this->estudioslegajo->store($ipersid, $ccentroestudios, $dfechainicio, $dfechatermino, $cgrado_titulo, $ccolegiaturanro);
                return $estudios;
                break;
            case 'actualizar':
                $estudios = $this->estudioslegajo->update($iperstipoestudid, $ccentroestudios, $dfechainicio, $dfechatermino, $cgrado_titulo, $ccolegiaturanro);
                return $estudios;
            case 'eliminar':
                $estudios = $this->estudioslegajo->delete($iperstipoestudid);
                return $estudios;
                break;
        }
    }
    ...
    public function ui_legajo_capacitacion()
    {
        $accion        = trim($this->input->post('accion'));
        $indiv_id        = trim($this->input->post('indiv_id'));
        $ipersid        = trim($this->input->post('ipersid'));
        $iperstipocapacid        = trim($this->input->post('iperstipocapacid'));
    ...
    public function ui_legajo_expLaboral()
    {
        ...
        $indiv_id        = trim($this->input->post('indiv_id'));
        $ipersid        = trim($this->input->post('ipersid'));
        $iexp_laboralid        = trim($this->input->post('iexp_laboralid'));
    ...
    public function ui_legajo_meritos()
    {
        ...
        $indiv_id        = trim($this->input->post('indiv_id'));
        $ipersid        = trim($this->input->post('ipersid'));
        $imeritosid        = trim($this->input->post('imeritosid'));
    ...
    public function meritos_accion()
    {
        $accion = $this->input->post('accion');
        $ipersid = $this->input->post('ipersid');
        ...
        switch ($accion) {
            case 'agregar':
                $meritos = $this->meritoslegajo->store($ipersid, $ctipomerito, $cdocumentotipo, $cdocumentonro, $cdocumentofecha, $cmotivo);
                return $meritos;
                break;
            case 'actualizar':
    ...
    public function ui_legajo_demeritos()
    {
        ...
        $indiv_id        = trim($this->input->post('indiv_id'));
        $ipersid        = trim($this->input->post('ipersid'));
        $idemeritosid        = trim($this->input->post('idemeritosid'));
    ...
    public function demeritos_accion()
    {
        $accion = $this->input->post('accion');
        $ipersid = $this->input->post('ipersid');
        ...
        switch ($accion) {
            case 'agregar':
                $demeritos = $this->demeritoslegajo->store($ipersid, $sancion, $cdocumentoresolucion, $cdocumentonro, $cfecha_ini, $cfecha_fin);
                return $demeritos;
                break;
            case 'actualizar':
    ...
    public function ui_legajo_cargaFam()
    {
        $accion        = trim($this->input->post('accion'));
        $indiv_id        = trim($this->input->post('indiv_id'));
        $ipersid        = trim($this->input->post('ipersid'));
        $icarga_familiarid        = trim($this->input->post('icarga_familiarid'));
    ...
    public function cargaFam_accion()
    {
        $accion = $this->input->post('accion');
        $ipersid = $this->input->post('ipersid');

        $cape_nom_conyug = $this->input->post('cape_nom_conyug');
        $ccel_conyug = $this->input->input->post('ccel_conyug');
        $cape_nom_hijos = $this->input->post('cape_nom_hijos');
        $cfechanac_hijos = $this->input->post('cfechanac_hijos');
        $ape_nom_padre = $this->input->post('ape_nom_padre');
        $cel_padre = $this->input->post('cel_padre');
        $ape_nom_madre = $this->input->post('ape_nom_madre');
        $cel_madre = $this->input->post('cel_madre');

        switch ($accion) {
            case 'agregar':
                $cargaFam = $this->familiarlegajo->store($ipersid, $cape_nom_conyug, $ccel_conyug, $cape_nom_hijos, $cfechanac_hijos, $ape_nom_padre, $cel_padre, $ape_nom_madre, $cel_madre);
                return $cargaFam;
                break;
            case 'actualizar':
    ...
    public function ui_legajo_accion()
    {
        $ipersid        = trim($this->input->post('ipersid'));
        $dfechainicio   = trim($this->input->post('dfechainicio'));
        $this->load->view(
            'escalafon/v_legajo_info',
            array(
                'ipersid' => $ipersid,
    ...
    public function ui_full_legajo()
    {
        $emp_key        = trim($this->input->post('empkey'));
        $emp_id         = $this->persona->get_id($emp_key);
        ...
        if ($this->user->has_key('TRABAJADOR_MODIFICAR_INFO_PERSONAL')) {
            ...
            if ($data['hascbanco'] == '1') {
                ...
                if (trim($info_actual['ebanco_id']) != $data['banco']  || trim($info_actual['pecd_cuentabancaria']) != trim($data['bancocod'])) {
                    ...
                    $banco_id = $this->banco->get_id($data['banco']);
                    $this->persona->add_cuentadeposito($emp_id, array(
                        'ebanco_id' => $banco_id,
                        'pecd_cuentabancaria' => $data['bancocod']
                    ));
                }
          ...
          if($data['hasessalud']=='1')
          {
            ...
            if (
                $info_actual['pentip_id'] != $data['tipopension']
    ...
    public function registrar_comision()
    {
        ...
        if ($this->user->has_key('TRABAJADOR_COMISIONSERVICIO_EDITAR')) {
            ...
            $licencias_del_trabajador =  $this->licencia->getLicenciasDia(array(
                'indiv_id' => $emp_id,
    ...
    public function explorar_por_fechas($tipo = '')
    {
        ...
        if ($tipo == '') {
            ...
            $this->load->view('escalafon/v_expxfechas_center', array(
                'tipo' => $data['tipoview'],
    ...
    public function gestion_rapida_de_conceptos()
    {
        ...
        $this->load->view('escalafon/v_ar_gestionar_conceptos', array(
            'conceptos'  => $conceptos,
            'persona'    => $pers_info
        ));
    }
