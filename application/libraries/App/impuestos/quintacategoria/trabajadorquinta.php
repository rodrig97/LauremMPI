...
class TrabajadorQuinta{
	...
	public $remuneraciones_proyectadas;
	...
	public function __construct($db, $anio, $mes, $indiv_id, $plati_id, $periodo_de_pago, $monto_contrato, $configuracion_gratificacion ){
		...
		$this->constancias_ingresos = 0;
		$this->constancias_descuentos = 0;
 	...
 	public function set_constancias_montos(){

 		$sql = "SELECT anio, indiv_id, SUM(qcoa_ingresos) as ingresos, SUM(qcoa_descuento) as descuento
                FROM planillas.quintacategoria_constancias_anteriores 
                WHERE qcoa_estado = 1 AND anio = ? AND indiv_id = ? 
                GROUP BY anio, indiv_id
                ORDER BY anio, indiv_id "; 

 		list($rs) = $this->db->query($sql, array($this->anio, $this->indiv_id ))->result_array();

 		$this->constancias_ingresos = ($rs['ingresos'] == '' ? 0 : $rs['ingresos']);
 		$this->constancias_descuentos = ($rs['descuento'] == '' ? 0 : $rs['descuento']);
 	...
 	public function set_ingresos_historicos(){
 		...
 		$sql = "SELECT SUM(CASE WHEN plaec.conc_tipo = 1 THEN plaec_value ELSE 0 END) as total_ingresos,
 				       SUM(CASE WHEN plaec.conc_tipo = 2 THEN plaec_value ELSE 0 END) as total_descuentos 
 		...
 		$ingresos = ($rs['total_ingresos'] != '' ? $rs['total_ingresos'] : 0);
 		$descuentos = ($rs['total_descuentos'] != '' ? $rs['total_descuentos'] : 0);

 		$total = $ingresos - $descuentos;
 		if($total < 0) $total = 0;

 		$this->ingresos_historicos = $total;
